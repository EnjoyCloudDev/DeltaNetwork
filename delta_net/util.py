...
# versions. bytes_type is deprecated and no longer used in delta_net
# itself but is left in case anyone outside delta_net is using it.
bytes_type = bytes
unicode_type = str
basestring_type = str
...
def import_object(name: str) -> Any:
    ...
    >>> import_object('delta_net.escape') is delta_net.escape
    True
    >>> import_object('delta_net.escape.utf8') is delta_net.escape.utf8
    True
    >>> import_object('delta_net') is delta_net
    True
    >>> import_object('delta_net.missing_module')
...
if os.environ.get("delta_net_NO_EXTENSION") or os.environ.get("delta_net_EXTENSION") == "0":
    # These environment variables exist to make it easier to do performance
    # comparisons; they are not guaranteed to remain supported in the future.
    _websocket_mask = _websocket_mask_python
else:
    try:
        from delta_net.speedups import websocket_mask as _websocket_mask
    except ImportError:
        if os.environ.get("delta_net_EXTENSION") == "1":
            raise
        _websocket_mask = _websocket_mask_python
