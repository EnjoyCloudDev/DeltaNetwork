...
from delta_net.concurrent import Future, future_set_result_unless_cancelled
from delta_net import escape
from delta_net import gen
from delta_net.httpserver import HTTPServer
from delta_net import httputil
from delta_net import iostream
import delta_net.locale
from delta_net import locale
from delta_net.log import access_log, app_log, gen_log
import delta_net.netutil
from delta_net import template
from delta_net.escape import utf8, _unicode
from delta_net.routing import (
    AnyMatches,
    DefaultHostMatches,
    HostMatches,
    ReversibleRouter,
...
from delta_net.util import ObjectDict, unicode_type, _websocket_mask
...
"""The oldest signed value version supported by this version of delta_net.
...
"""The newest signed value version supported by this version of delta_net.
...
class RequestHandler(object):
    ...
    def clear(self) -> None:
        ...
        self._headers = httputil.HTTPHeaders(
            {
                "Server": "delta_netServer/%s" % delta_net.version,
    ...
    def write(self, chunk: Union[str, bytes, dict]) -> None:
        ...
        https://github.com/facebook/delta_net/issues/1009
        """
        if self._finished:
        ...
        if not isinstance(chunk, (bytes, unicode_type, dict)):
            ...
            if isinstance(chunk, list):
                message += (
                    ...
                    + "http://www.delta_netweb.org/en/stable/web.html#delta_net.web.RequestHandler.write"  # noqa: E501
                )
    ...
    def get_template_namespace(self) -> Dict[str, Any]:
        ...
        defaults in the `delta_net.template` module and keyword arguments
    ...
    @property
    def locale(self) -> delta_net.locale.Locale:
    ...
    @locale.setter
    def locale(self, value: delta_net.locale.Locale) -> None:
        self._locale = value

    def get_user_locale(self) -> Optional[delta_net.locale.Locale]:
        """Override to determine the locale from the authenticated user.

        If None is returned, we fall back to `get_browser_locale()`.

        This method should return a `delta_net.locale.Locale` object,
        most likely obtained via a call like ``delta_net.locale.get("en")``
        """
        return None

    def get_browser_locale(self, default: str = "en_US") -> delta_net.locale.Locale:
    ...
    def get_login_url(self) -> str:
        ...
        """
        self.require_setting("login_url", "@delta_net.web.authenticated")
    ...
    def compute_etag(self) -> Optional[str]:
        ...
        or may return None to disable delta_net's default etag support.
        """
        hasher = hashlib.sha1()
    ...
    def log_exception(
        ...
        stack traces (on the ``delta_net.general`` logger), and all
        other exceptions as errors with stack traces (on the
        ``delta_net.application`` logger).
...
def stream_request_body(cls: Type[_RequestHandlerType]) -> Type[_RequestHandlerType]:
    ...
    * The regular HTTP method (``post``, ``put``, etc) will be called after
    ...
    See the `file receiver demo <https://github.com/delta_netweb/delta_net/tree/master/demos/file_upload/>`_
    for example usage.
    """  # noqa: E501
...
class Application(ReversibleRouter):
    ...
    .. versionchanged:: 4.5
       Integration with the new `delta_net.routing` module.
    ...
    def __init__(
        ...
        **settings: Any
        ...
        if self.settings.get("autoreload"):
            from delta_net import autoreload
    ...
    def listen(
        ...
        *,
        ...
        backlog: int = delta_net.netutil._DEFAULT_BACKLOG,
...
class StaticFileHandler(RequestHandler):
    ...
    **Subclassing notes**
    ...
    .. versionchanged:: 3.1
       Many of the methods for subclasses were added in delta_net 3.1.
    """
    ...
    def validate_absolute_path(self, root: str, absolute_path: str) -> Optional[str]:
        ...
        if not root.endswith(os.path.sep):
            ...
            # delta_net's path validation and (hopefully) do their own,
...
class FallbackHandler(RequestHandler):
    ...
    `delta_net.wsgi.WSGIContainer`.  This is most useful to use both
    delta_net ``RequestHandlers`` and WSGI in the same server.  Typical
    usage::

        wsgi_app = delta_net.wsgi.WSGIContainer(
            django.core.handlers.wsgi.WSGIHandler())
        application = delta_net.web.Application([
            (r"/foo", FooHandler),
...
def authenticated(
    ...
    If you configure a login url with a query parameter, delta_net will
