...
import sys
import struct
import delta_net.escape
import delta_net.web
from urllib.parse import urlparse
import zlib

from delta_net.concurrent import Future, future_set_result_unless_cancelled
from delta_net.escape import utf8, native_str, to_unicode
from delta_net import gen, httpclient, httputil
from delta_net.ioloop import IOLoop, PeriodicCallback
from delta_net.iostream import StreamClosedError, IOStream
from delta_net.log import gen_log, app_log
from delta_net.netutil import Resolver
from delta_net import simple_httpclient
from delta_net.queues import Queue
from delta_net.tcpclient import TCPClient
from delta_net.util import _websocket_mask
...
class WebSocketHandler(delta_net.web.RequestHandler):
    ...
    `~delta_net.web.RequestHandler.set_default_headers` or
    `~delta_net.web.RequestHandler.prepare`.
    ...
    .. testcode::
      ...
      class EchoWebSocket(delta_net.websocket.WebSocketHandler):
          def open(self):
    ...
    message-based. Consequently, most of the delta_net HTTP facilities
    ...
    security hole, so since delta_net 4.0 `WebSocketHandler` requires
    ...
    def write_message(
        ...
        if isinstance(message, dict):
            message = delta_net.escape.json_encode(message)
    ...
    def open(self, *args: str, **kwargs: str) -> Optional[Awaitable[None]]:
        ...
        The arguments to `open` are extracted from the `delta_net.web.URLSpec`
        regular expression, just like the arguments to
        `delta_net.web.RequestHandler.get`.
    ...
    def check_origin(self, origin: str) -> bool:
        ...
        delta_net 4.0), simply override this method to always return ``True``::
...
class _PerMessageDeflateCompressor(object):
    def __init__(
        ...
        if (
            ...
            self._compression_level = delta_net.web.GZipContentEncoding.GZIP_LEVEL
        else:
...
class WebSocketProtocol13(WebSocketProtocol):
    ...
    def write_message(
        ...
        if isinstance(message, dict):
            message = delta_net.escape.json_encode(message)
        message = delta_net.escape.utf8(message)
        ...
        if self._compressor:
            ...
            flags |= self.RSV1
        # For historical reasons, write methods in delta_net operate in a semi-synchronous
