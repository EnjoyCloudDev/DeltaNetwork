...
from delta_net.escape import native_str
from delta_net.http1connection import HTTP1ServerConnection, HTTP1ConnectionParameters
from delta_net import httputil
from delta_net import iostream
from delta_net import netutil
from delta_net.tcpserver import TCPServer
from delta_net.util import Configurable
...
class HTTPServer(TCPServer, Configurable, httputil.HTTPServerConnectionDelegate):
    ...
    are useful when running delta_net behind a reverse proxy or load
    balancer.  The ``protocol`` argument can also be set to ``https``
    if delta_net is run behind an SSL-decoding proxy that does not set one of
    the supported ``xheaders``.

    By default, when parsing the ``X-Forwarded-For`` header, delta_net will
    ...
    initialization methods are defined on `delta_net.tcpserver.TCPServer`):

    1. `~delta_net.tcpserver.TCPServer.listen`: single-process::
       ...
       In many cases, `delta_net.web.Application.listen` can be used to avoid
    ...
    2. `~delta_net.tcpserver.TCPServer.add_sockets`: multi-process::

            sockets = bind_sockets(8888)
            delta_net.process.fork_processes(0)
       ...
       `delta_net.process.fork_processes` to run a multi-process service with all
       ...
       sockets in some way other than `~delta_net.netutil.bind_sockets`.

       Note that when using this pattern, nothing that touches the event loop
       can be run before ``fork_processes``.

    3. `~delta_net.tcpserver.TCPServer.bind`/`~delta_net.tcpserver.TCPServer.start`:
    ...
    .. versionchanged:: 4.2
       `HTTPServer` is now a subclass of `delta_net.util.Configurable`.
