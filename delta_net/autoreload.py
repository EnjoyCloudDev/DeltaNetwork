...
if __name__ == "__main__":
    ...
    # as possible - if we introduced a delta_net.sys or delta_net.os
...
from delta_net import ioloop
from delta_net.log import gen_log
from delta_net import process
from delta_net.util import exec_in
...
def start(check_time: int = 500) -> None:
    ...
    if len(_io_loops) > 1:
        gen_log.warning("delta_net.autoreload started more than once in the same process")
...
_USAGE = """\
Usage:
  python -m delta_net.autoreload -m module.to.run [args...]
  python -m delta_net.autoreload path/to/script.py [args...]
"""
...
def main() -> None:
    ...
    Scripts may be specified by filename or module name::
        ...
        python -m delta_net.autoreload -m delta_net.test.runtests
        python -m delta_net.autoreload delta_net/test/runtests.py

    Running a script with this wrapper is similar to calling
    `delta_net.autoreload.wait` at the end of the script, but this wrapper
    ...
    import delta_net.autoreload

    global _autoreload_is_main
    global _original_argv, _original_spec
    delta_net.autoreload._autoreload_is_main = _autoreload_is_main = True
    original_argv = sys.argv
    delta_net.autoreload._original_argv = _original_argv = original_argv
    original_spec = getattr(sys.modules["__main__"], "__spec__", None)
    delta_net.autoreload._original_spec = _original_spec = original_spec
    sys.argv = sys.argv[:]
