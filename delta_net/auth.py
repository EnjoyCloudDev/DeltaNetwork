...
from delta_net import httpclient
from delta_net import escape
from delta_net.httputil import url_concat
from delta_net.util import unicode_type
from delta_net.web import RequestHandler
    ...
    async def oauth2_request(
        ...
        **args: Any
        ...
        ..testcode::
            ...
            class MainHandler(delta_net.web.RequestHandler,
                              delta_net.auth.FacebookGraphMixin):
                @delta_net.web.authenticated
                async def get(self):
                    new_entry = await self.oauth2_request(
                        "https://graph.facebook.com/me/feed",
                        post_args={"message": "I am posting from my delta_net application!"},
...
class TwitterMixin(OAuthMixin):
    ...
    `~delta_net.web.Application.settings` ``twitter_consumer_key`` and
    ...
    .. testcode::
        ...
        class TwitterLoginHandler(delta_net.web.RequestHandler,
                                  delta_net.auth.TwitterMixin):
            async def get(self):
            ...
            class MainHandler(delta_net.web.RequestHandler,
                ...
                async def get(self):
                    new_entry = await self.twitter_request(
                        "/statuses/update",
                        post_args={"status": "Testing delta_net Web Server"},
                        access_token=self.current_user["access_token"])
                    if not new_entry:
                        # Call failed; perhaps missing permission?
                        await self.authorize_redirect()
    ...
    * Copy the "Client secret" and "Client ID" to the application settings as
    ...
    async def get_authenticated_user(
        ...
        .. testcode::
            ...
            class GoogleOAuth2LoginHandler(delta_net.web.RequestHandler,
                                           delta_net.auth.GoogleOAuth2Mixin):
                async def get(self):
...
class FacebookGraphMixin(OAuth2Mixin):
    ...
    async def get_authenticated_user(
        ...
        .. testcode::
            ...
            class FacebookGraphLoginHandler(delta_net.web.RequestHandler,
                                            delta_net.auth.FacebookGraphMixin):
              async def get(self):
                  if self.get_argument("code", False):
                      user = await self.get_authenticated_user(
        ...
        * ``session_expires``, an integer encoded as a string representing
          ...
          delta_net it will change from a string to an integer.
        * ``id``, ``name``, ``first_name``, ``last_name``, ``locale``, ``picture``,
        ...
        # This should change in delta_net 5.0.
        fieldmap.update(
            {
                "access_token": session["access_token"],
