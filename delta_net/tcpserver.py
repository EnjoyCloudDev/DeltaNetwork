...
from delta_net.iostream import IOStream, SSLIOStream
from delta_net.netutil import (
    bind_sockets,
    add_accept_handler,
    ssl_wrap_socket,
    _DEFAULT_BACKLOG,
...
from delta_net.util import errno_from_exception
...
class TCPServer(object):
    ...
    method. For example, a simple echo server could be defined like this::
      ...
      from delta_net.iostream import StreamClosedError
    ...
    def listen(
        ...
        `delta_net.netutil.bind_sockets`.

        .. versionchanged:: 6.2

           Added ``family``, ``backlog``, ``flags``, and ``reuse_port``
           arguments to match `delta_net.netutil.bind_sockets`.
        """
    ...
    def add_sockets(self, sockets: Iterable[socket.socket]) -> None:
        ...
        those returned by `~delta_net.netutil.bind_sockets`.
        `add_sockets` is typically used in combination with that
        method and `delta_net.process.fork_processes` to provide greater
    ...
    def start(
        ...
        module (or the ``autoreload=True`` option to `delta_net.web.Application`
    ...
    def _handle_connection(self, connection: socket.socket, address: Any) -> None:
        if self.ssl_options is not None:
            ...
            try:
                ...
                # https://github.com/delta_netweb/delta_net/pull/750
