...
from delta_net.ioloop import IOLoop, _Selectable
...
class AsyncIOLoop(BaseAsyncIOLoop):
    ...
    This class follows the usual delta_net semantics for creating new
...
def to_delta_net_future(asyncio_future: asyncio.Future) -> asyncio.Future:
    """Convert an `asyncio.Future` to a `delta_net.concurrent.Future`.

    .. versionadded:: 4.1

    .. deprecated:: 5.0
       delta_net ``Futures`` have been merged with `asyncio.Future`,
       so this method is now a no-op.
    """
    return asyncio_future


def to_asyncio_future(delta_net_future: asyncio.Future) -> asyncio.Future:
    """Convert a delta_net yieldable object to an `asyncio.Future`.

    .. versionadded:: 4.1

    .. versionchanged:: 4.3
       Now accepts any yieldable object, not just
       `delta_net.concurrent.Future`.

    .. deprecated:: 5.0
       delta_net ``Futures`` have been merged with `asyncio.Future`,
       so this method is now equivalent to `delta_net.gen.convert_yielded`.
    """
    return convert_yielded(delta_net_future)
...
class AnyThreadEventLoopPolicy(_BasePolicy):  # type: ignore
    ...
    behavior of delta_net versions prior to 5.0 (or 5.0 on Python 2).
...
class AddThreadSelectorEventLoop(asyncio.AbstractEventLoop):
    ...
    This class is used automatically by delta_net; applications should not need
    ...
    def __del__(self) -> None:
        ...
        # start and stop the event loop, no objects created in delta_net
